...
test_that("UDSVERLC returns an object inheriting both UDSVERLC and npsych_scores", {
  result <- UDSVERLC()
...
test_that("UDSVERLC sets class with UDSVERLC first, npsych_scores second", {
  result <- UDSVERLC()

  expect_equal(class(result), c("UDSVERLC", "npsych_scores"))
})
...
suppressMessages(.setup_UDSVERLC_versions())


test_that(".setup_UDSVERLC_versions registers the expected methods", {
  methods <- list_std_methods(UDSVERLC())
...
test_that(".setup_UDSVERLC_versions registers the 'nacc' norms version", {
  expect_true("nacc" %in% list_method_versions(UDSVERLC(), "norms"))
})

test_that(".setup_UDSVERLC_versions registers the 'updated' norms version", {
  expect_true("updated" %in% list_method_versions(UDSVERLC(), "norms"))
})
...
test_that(".setup_UDSVERLC_versions registers the 'updated_2024.06' regression version", {
  expect_true(
    "updated_2024.06" %in% list_method_versions(UDSVERLC(), "regression")
  )
})

test_that(".setup_UDSVERLC_versions registers the 'updated_2025.06' regression version", {
  expect_true(
    "updated_2025.06" %in% list_method_versions(UDSVERLC(), "regression")
  )
})

test_that(".setup_UDSVERLC_versions registers the 'nacc' regression version", {
  expect_true("nacc" %in% list_method_versions(UDSVERLC(), "regression"))
})
...
test_that(".setup_UDSVERLC_versions sets the default method to 'regression' / 'updated_2025.06'", {
  default <- get_std_defaults(UDSVERLC())

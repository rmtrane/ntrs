...
test_that("MEMUNITS returns an object inheriting both MEMUNITS and test_scores", {
  result <- MEMUNITS()
...
test_that("MEMUNITS sets class with MEMUNITS first, test_scores second", {
  result <- MEMUNITS()

  expect_equal(class(result), c("MEMUNITS", "test_scores"))
})
...
test_that(".setup_MEMUNITS_versions registers the expected methods", {
  methods <- get_std_methods(MEMUNITS())
...
test_that(".setup_MEMUNITS_versions registers the 'updated_2024.06' regression version", {
  expect_true("updated_2024.06" %in% get_versions(MEMUNITS(), "regression"))
})

test_that(".setup_MEMUNITS_versions registers the 'updated_2025.06' regression version", {
  expect_true("updated_2025.06" %in% get_versions(MEMUNITS(), "regression"))
})

test_that(".setup_MEMUNITS_versions registers the 'nacc_legacy' regression version", {
  expect_true("nacc_legacy" %in% get_versions(MEMUNITS(), "regression"))
})
...
test_that(".setup_MEMUNITS_versions sets the default method to 'regression' / 'nacc_legacy'", {
  default <- get_default_method(MEMUNITS())

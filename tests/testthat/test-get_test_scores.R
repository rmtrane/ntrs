test_that("'get_test_scores()' picks up new method", {
  assign(
  ...
  expect_false("new_class" %in% get_test_scores())
  ...
  expect_true("new_class" %in% get_test_scores())
})

# Registry source ----

test_that("get_test_scores() finds subclass with registered norms version", {
  expect_true("MOCATOTS" %in% get_test_scores())
})

test_that("get_test_scores() finds subclass with registered regression version", {
  ...
  expect_true("reg_only_class" %in% get_test_scores())
})

test_that("get_test_scores() does not return phantom registry entries failing .is_valid_test_scores()", {
  ...
  expect_false("phantom_class" %in% get_test_scores())
})

# S3 scan source ----

test_that("get_test_scores() does not return class scraped from S3 name that fails .is_valid_test_scores()", {
  assign(
  ...
  expect_false("not_a_real_class" %in% get_test_scores())
})

# Combined behaviour ----

test_that("get_test_scores() does not return duplicates for classes in both registry and S3 scan", {
  # MOCATOTS is in the registry and has S3 methods in the loaded namespace
  result <- get_test_scores()
  expect_equal(sum(result == "MOCATOTS"), 1L)
})

test_that("get_test_scores() returns all known internal subclasses", {
  result <- get_test_scores()

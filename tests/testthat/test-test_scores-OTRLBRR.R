...
test_that("OTRLBRR returns an object inheriting both OTRLBRR and test_scores", {
  result <- OTRLBRR()
...
test_that("OTRLBRR sets class with OTRLBRR first, test_scores second", {
  result <- OTRLBRR()

  expect_equal(class(result), c("OTRLBRR", "test_scores"))
})
...
test_that(".setup_OTRLBRR_versions registers the expected methods", {
  methods <- get_std_methods(OTRLBRR())
...
test_that(".setup_OTRLBRR_versions registers the 'updated' norms version", {
  expect_true("updated" %in% get_versions(OTRLBRR(), "norms"))
})
...
test_that(".setup_OTRLBRR_versions registers the 'updated_2024.06' regression version", {
  expect_true("updated_2024.06" %in% get_versions(OTRLBRR(), "regression"))
})

test_that(".setup_OTRLBRR_versions registers the 'updated_2025.06' regression version", {
  expect_true("updated_2025.06" %in% get_versions(OTRLBRR(), "regression"))
})
...
test_that(".setup_OTRLBRR_versions sets the default method to 'regression' / 'updated_2025.06'", {
  default <- get_default_method(OTRLBRR())

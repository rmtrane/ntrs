...
test_that(".is_valid_std_method() returns TRUE for method implemented via test_scores generic", {
  # std_using_norms.test_scores exists, so this resolves via inheritance
...
# get_std_methods() tests ----

test_that("get_std_methods() returns norms and regression for MOCATOTS", {
  result <- get_std_methods(MOCATOTS())
  expect_true("norms" %in% result)
  expect_true("regression" %in% result)
})

test_that("get_std_methods() finds method defined in .GlobalEnv for existing class", {
  assign(
    "std_using_new_method",
    function(x, ...) UseMethod("std_using_new_method"),
    envir = .GlobalEnv
  ...
  expect_true("new_method" %in% get_std_methods(MOCATOTS()))
})

test_that("get_std_methods() finds method implemented via test_scores generic for new class", {
  assign(
    "new_class",
  ...
  # norms and regression are both implemented via std_using_*.test_scores,
  # but without versions. Hence, should not be surfaced by get_std_methods...
  result <- get_std_methods(new_class())
...
test_that("get_std_methods() does not return 'defaults' from registry", {
  expect_false("defaults" %in% get_std_methods(MOCATOTS()))
})

test_that("get_std_methods() does not return duplicates when method is in both registry and S3 scan", {
  result <- get_std_methods(MOCATOTS())
  expect_equal(sum(result == "norms"), 1L)
  expect_equal(sum(result == "regression"), 1L)
})

test_that("get_std_methods() returns empty character vector for class with no methods or registered versions", {
  assign(
    "bare_class",
    function(scores = numeric()) {
      test_scores(
  ...
  expect_equal(get_std_methods(bare_class()), character(0))
})

test_that(".is_valid_std_method() errors when test_class is not a test_scores object", {

...
test_that("OTRAILB returns an object inheriting both OTRAILB and test_scores", {
  result <- OTRAILB()
...
test_that("OTRAILB sets class with OTRAILB first, test_scores second", {
  result <- OTRAILB()

  expect_equal(class(result), c("OTRAILB", "test_scores"))
})
...
test_that(".setup_OTRAILB_versions registers the expected methods", {
  methods <- get_std_methods(OTRAILB())
...
test_that(".setup_OTRAILB_versions registers the 'updated' norms version", {
  expect_true("updated" %in% get_versions(OTRAILB(), "norms"))
})
...
test_that(".setup_OTRAILB_versions registers the 'updated_2024.06' regression version", {
  expect_true("updated_2024.06" %in% get_versions(OTRAILB(), "regression"))
})

test_that(".setup_OTRAILB_versions registers the 'updated_2025.06' regression version", {
  expect_true("updated_2025.06" %in% get_versions(OTRAILB(), "regression"))
})
...
test_that(".setup_OTRAILB_versions sets the default method to 'regression' / 'updated_2025.06'", {
  default <- get_default_method(OTRAILB())

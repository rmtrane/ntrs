...
test_that("UDSBENTC returns an object inheriting both UDSBENTC and test_scores", {
  result <- UDSBENTC()
...
test_that("UDSBENTC sets class with UDSBENTC first, test_scores second", {
  result <- UDSBENTC()

  expect_equal(class(result), c("UDSBENTC", "test_scores"))
})
...
test_that(".setup_UDSBENTC_versions registers the expected methods", {
  methods <- get_std_methods(UDSBENTC())
...
test_that(".setup_UDSBENTC_versions registers the 'nacc' norms version", {
  expect_true("nacc" %in% get_versions(UDSBENTC(), "norms"))
})

test_that(".setup_UDSBENTC_versions registers the 'updated' norms version", {
  expect_true("updated" %in% get_versions(UDSBENTC(), "norms"))
})
...
test_that(".setup_UDSBENTC_versions registers the 'updated_2024.06' regression version", {
  expect_true("updated_2024.06" %in% get_versions(UDSBENTC(), "regression"))
})

test_that(".setup_UDSBENTC_versions registers the 'updated_2025.06' regression version", {
  expect_true("updated_2025.06" %in% get_versions(UDSBENTC(), "regression"))
})

test_that(".setup_UDSBENTC_versions registers the 'nacc' regression version", {
  expect_true("nacc" %in% get_versions(UDSBENTC(), "regression"))
})
...
test_that(".setup_UDSBENTC_versions sets the default method to 'regression' / 'updated_2025.06'", {
  default <- get_default_method(UDSBENTC())

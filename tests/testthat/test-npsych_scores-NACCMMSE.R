...
test_that("NACCMMSE returns an object inheriting both NACCMMSE and npsych_scores", {
  result <- NACCMMSE()
...
test_that("NACCMMSE sets class with NACCMMSE first, npsych_scores second", {
  result <- NACCMMSE()

  expect_equal(class(result), c("NACCMMSE", "npsych_scores"))
})
...
test_that(".setup_NACCMMSE_versions registers the 'updated_2024.06' regression version", {
  expect_true(
    "updated_2024.06" %in% list_method_versions(NACCMMSE(), "regression")
  )
})

test_that(".setup_NACCMMSE_versions registers the 'updated_2025.06' regression version", {
  expect_true(
    "updated_2025.06" %in% list_method_versions(NACCMMSE(), "regression")
  )
})

test_that(".setup_NACCMMSE_versions registers the 'nacc_legacy' regression version", {
  expect_true("nacc_legacy" %in% list_method_versions(NACCMMSE(), "regression"))
})
...
test_that(".setup_NACCMMSE_versions sets the default method to 'regression' / 'nacc_legacy'", {
  default <- get_std_defaults(NACCMMSE())

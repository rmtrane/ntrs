...
test_that("UDSVERTN returns an object inheriting both UDSVERTN and npsych_scores", {
  result <- UDSVERTN()
...
test_that("UDSVERTN sets class with UDSVERTN first, npsych_scores second", {
  result <- UDSVERTN()

  expect_equal(class(result), c("UDSVERTN", "npsych_scores"))
})
...
test_that(".setup_UDSVERTN_versions registers the 'nacc' norms version", {
  expect_true("nacc" %in% list_method_versions(UDSVERTN(), "norms"))
})

test_that(".setup_UDSVERTN_versions registers the 'updated' norms version", {
  expect_true("updated" %in% list_method_versions(UDSVERTN(), "norms"))
})
...
test_that(".setup_UDSVERTN_versions registers the 'updated_2024.06' regression version", {
  expect_true(
    "updated_2024.06" %in% list_method_versions(UDSVERTN(), "regression")
  )
})

test_that(".setup_UDSVERTN_versions registers the 'updated_2025.06' regression version", {
  expect_true(
    "updated_2025.06" %in% list_method_versions(UDSVERTN(), "regression")
  )
})

test_that(".setup_UDSVERTN_versions registers the 'nacc' regression version", {
  expect_true("nacc" %in% list_method_versions(UDSVERTN(), "regression"))
})
...
test_that(".setup_UDSVERTN_versions sets the default method to 'regression' / 'updated_2025.06'", {
  default <- get_std_defaults(UDSVERTN())

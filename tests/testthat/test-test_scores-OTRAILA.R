...
test_that("OTRAILA returns an object inheriting both OTRAILA and test_scores", {
  result <- OTRAILA()
...
test_that("OTRAILA sets class with OTRAILA first, test_scores second", {
  result <- OTRAILA()

  expect_equal(class(result), c("OTRAILA", "test_scores"))
})
...
test_that(".setup_OTRAILA_versions registers the expected methods", {
  methods <- get_std_methods(OTRAILA())
...
test_that(".setup_OTRAILA_versions registers the 'updated' norms version", {
  expect_true("updated" %in% get_versions(OTRAILA(), "norms"))
})
...
test_that(".setup_OTRAILA_versions registers the 'updated_2024.06' regression version", {
  expect_true("updated_2024.06" %in% get_versions(OTRAILA(), "regression"))
})

test_that(".setup_OTRAILA_versions registers the 'updated_2025.06' regression version", {
  expect_true("updated_2025.06" %in% get_versions(OTRAILA(), "regression"))
})
...
test_that(".setup_OTRAILA_versions sets the default method to 'regression' / 'updated_2025.06'", {
  default <- get_default_method(OTRAILA())

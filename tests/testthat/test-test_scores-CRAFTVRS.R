...
test_that("CRAFTVRS returns an object inheriting both CRAFTVRS and test_scores", {
  result <- CRAFTVRS()
...
test_that("CRAFTVRS sets class with CRAFTVRS first, test_scores second", {
  result <- CRAFTVRS()

  expect_equal(class(result), c("CRAFTVRS", "test_scores"))
})
...
test_that(".setup_CRAFTVRS_versions registers the expected methods", {
  methods <- get_std_methods(CRAFTVRS())
...
test_that(".setup_CRAFTVRS_versions registers the 'nacc' norms version", {
  expect_true("nacc" %in% get_versions(CRAFTVRS(), "norms"))
})

test_that(".setup_CRAFTVRS_versions registers the 'updated' norms version", {
  expect_true("updated" %in% get_versions(CRAFTVRS(), "norms"))
})
...
test_that(".setup_CRAFTVRS_versions registers the 'updated_2024.06' regression version", {
  expect_true("updated_2024.06" %in% get_versions(CRAFTVRS(), "regression"))
})

test_that(".setup_CRAFTVRS_versions registers the 'updated_2025.06' regression version", {
  expect_true("updated_2025.06" %in% get_versions(CRAFTVRS(), "regression"))
})

test_that(".setup_CRAFTVRS_versions registers the 'nacc' regression version", {
  expect_true("nacc" %in% get_versions(CRAFTVRS(), "regression"))
})
...
test_that(".setup_CRAFTVRS_versions sets the default method to 'regression' / 'updated_2025.06'", {
  default <- get_default_method(CRAFTVRS())

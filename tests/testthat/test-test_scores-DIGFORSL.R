...
test_that("DIGFORSL returns an object inheriting both DIGFORSL and test_scores", {
  result <- DIGFORSL()
...
test_that("DIGFORSL sets class with DIGFORSL first, test_scores second", {
  result <- DIGFORSL()

  expect_equal(class(result), c("DIGFORSL", "test_scores"))
})
...
test_that(".setup_DIGFORSL_versions registers the expected methods", {
  methods <- get_std_methods(DIGFORSL())
...
test_that(".setup_DIGFORSL_versions registers the 'nacc' norms version", {
  expect_true("nacc" %in% get_versions(DIGFORSL(), "norms"))
})

test_that(".setup_DIGFORSL_versions registers the 'updated' norms version", {
  expect_true("updated" %in% get_versions(DIGFORSL(), "norms"))
})
...
test_that(".setup_DIGFORSL_versions registers the 'updated_2024.06' regression version", {
  expect_true("updated_2024.06" %in% get_versions(DIGFORSL(), "regression"))
})

test_that(".setup_DIGFORSL_versions registers the 'updated_2025.06' regression version", {
  expect_true("updated_2025.06" %in% get_versions(DIGFORSL(), "regression"))
})

test_that(".setup_DIGFORSL_versions registers the 'nacc' regression version", {
  expect_true("nacc" %in% get_versions(DIGFORSL(), "regression"))
})
...
test_that(".setup_DIGFORSL_versions sets the default method to 'regression' / 'updated_2025.06'", {
  default <- get_default_method(DIGFORSL())

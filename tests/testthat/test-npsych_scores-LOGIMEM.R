...
test_that("LOGIMEM returns an object inheriting both LOGIMEM and npsych_scores", {
  result <- LOGIMEM()
...
test_that("LOGIMEM sets class with LOGIMEM first, npsych_scores second", {
  result <- LOGIMEM()

  expect_equal(class(result), c("LOGIMEM", "npsych_scores"))
})
...
test_that(".setup_LOGIMEM_versions registers the 'updated_2024.06' regression version", {
  expect_true(
    "updated_2024.06" %in% list_method_versions(LOGIMEM(), "regression")
  )
})

test_that(".setup_LOGIMEM_versions registers the 'updated_2025.06' regression version", {
  expect_true(
    "updated_2025.06" %in% list_method_versions(LOGIMEM(), "regression")
  )
})

test_that(".setup_LOGIMEM_versions registers the 'nacc_legacy' regression version", {
  expect_true("nacc_legacy" %in% list_method_versions(LOGIMEM(), "regression"))
})
...
test_that(".setup_LOGIMEM_versions sets the default method to 'regression' / 'nacc_legacy'", {
  default <- get_std_defaults(LOGIMEM())

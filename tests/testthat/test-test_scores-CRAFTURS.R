...
test_that("CRAFTURS returns an object inheriting both CRAFTURS and test_scores", {
  result <- CRAFTURS()
...
test_that("CRAFTURS sets class with CRAFTURS first, test_scores second", {
  result <- CRAFTURS()

  expect_equal(class(result), c("CRAFTURS", "test_scores"))
})
...
test_that(".setup_CRAFTURS_versions registers the expected methods", {
  methods <- get_std_methods(CRAFTURS())
...
test_that(".setup_CRAFTURS_versions registers the 'nacc' norms version", {
  expect_true("nacc" %in% get_versions(CRAFTURS(), "norms"))
})

test_that(".setup_CRAFTURS_versions registers the 'updated' norms version", {
  expect_true("updated" %in% get_versions(CRAFTURS(), "norms"))
})
...
test_that(".setup_CRAFTURS_versions registers the 'updated_2024.06' regression version", {
  expect_true("updated_2024.06" %in% get_versions(CRAFTURS(), "regression"))
})

test_that(".setup_CRAFTURS_versions registers the 'updated_2025.06' regression version", {
  expect_true("updated_2025.06" %in% get_versions(CRAFTURS(), "regression"))
})

test_that(".setup_CRAFTURS_versions registers the 'nacc' regression version", {
  expect_true("nacc" %in% get_versions(CRAFTURS(), "regression"))
})
...
test_that(".setup_CRAFTURS_versions sets the default method to 'regression' / 'updated_2025.06'", {
  default <- get_default_method(CRAFTURS())

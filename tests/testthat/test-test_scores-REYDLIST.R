...
# NOTE: REYDLIST() does not pass `codes`, so test_scores() uses its
...
# validate_test_scores() is updated.

test_that("REYDLIST returns an object inheriting both REYDLIST and test_scores", {
  result <- REYDLIST()
...
test_that("REYDLIST sets class with REYDLIST first, test_scores second", {
  result <- REYDLIST()

  expect_equal(class(result), c("REYDLIST", "test_scores"))
})
...
test_that(".setup_REYDLIST_versions registers the expected methods", {
  methods <- get_std_methods(REYDLIST())
...
test_that(".setup_REYDLIST_versions registers the 'ravlt_trials' norms version", {
  expect_true("ravlt_trials" %in% get_versions(REYDLIST(), "norms"))
})

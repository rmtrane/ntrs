...
test_that("BOSTON returns an object inheriting both BOSTON and test_scores", {
  result <- BOSTON()
...
test_that("BOSTON sets class with BOSTON first, test_scores second", {
  result <- BOSTON()

  expect_equal(class(result), c("BOSTON", "test_scores"))
})
...
test_that(".setup_BOSTON_versions registers the expected methods", {
  methods <- get_std_methods(BOSTON())
...
test_that(".setup_BOSTON_versions registers the 'updated_2024.06' regression version", {
  expect_true("updated_2024.06" %in% get_versions(BOSTON(), "regression"))
})

test_that(".setup_BOSTON_versions registers the 'updated_2025.06' regression version", {
  expect_true("updated_2025.06" %in% get_versions(BOSTON(), "regression"))
})

test_that(".setup_BOSTON_versions registers the 'nacc_legacy' regression version", {
  expect_true("nacc_legacy" %in% get_versions(BOSTON(), "regression"))
})
...
test_that(".setup_BOSTON_versions sets the default method to 'regression' / 'nacc_legacy'", {
  default <- get_default_method(BOSTON())

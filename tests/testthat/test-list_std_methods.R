...
test_that("list_std_methods() finds new method", {
  assign(
    "std_using_new_method",
...
test_that("list_std_methods() dismisses method implemented via npsych_scores generic for new class", {
  ...
  # However, the method is still technically valid, so
  # S7::method() should find it without error
  expect_no_error(S7::method(std_using_norms, new_class))
})
...
test_that("list_std_methods() works as intended when no versions are registered", {
  ...
  S7::method(std_using_new_method, new_class) <- function(scores, ...) {
    "specific method"
  }

...
test_that("UDSVERFC returns an object inheriting both UDSVERFC and test_scores", {
  result <- UDSVERFC()
...
test_that("UDSVERFC sets class with UDSVERFC first, test_scores second", {
  result <- UDSVERFC()

  expect_equal(class(result), c("UDSVERFC", "test_scores"))
})
...
test_that(".setup_UDSVERFC_versions registers the expected methods", {
  methods <- get_std_methods(UDSVERFC())
...
test_that(".setup_UDSVERFC_versions registers the 'nacc' norms version", {
  expect_true("nacc" %in% get_versions(UDSVERFC(), "norms"))
})

test_that(".setup_UDSVERFC_versions registers the 'updated' norms version", {
  expect_true("updated" %in% get_versions(UDSVERFC(), "norms"))
})
...
test_that(".setup_UDSVERFC_versions registers the 'updated_2024.06' regression version", {
  expect_true("updated_2024.06" %in% get_versions(UDSVERFC(), "regression"))
})

test_that(".setup_UDSVERFC_versions registers the 'updated_2025.06' regression version", {
  expect_true("updated_2025.06" %in% get_versions(UDSVERFC(), "regression"))
})

test_that(".setup_UDSVERFC_versions registers the 'nacc' regression version", {
  expect_true("nacc" %in% get_versions(UDSVERFC(), "regression"))
})
...
test_that(".setup_UDSVERFC_versions sets the default method to 'regression' / 'updated_2025.06'", {
  default <- get_default_method(UDSVERFC())

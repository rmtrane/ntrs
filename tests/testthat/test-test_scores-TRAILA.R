...
test_that("TRAILA returns an object inheriting both TRAILA and test_scores", {
  result <- TRAILA()
...
test_that("TRAILA sets class with TRAILA first, test_scores second", {
  result <- TRAILA()

  expect_equal(class(result), c("TRAILA", "test_scores"))
})
...
test_that(".setup_TRAILA_versions registers the expected methods", {
  methods <- get_std_methods(TRAILA())
...
test_that(".setup_TRAILA_versions registers the 'nacc' norms version", {
  expect_true("nacc" %in% get_versions(TRAILA(), "norms"))
})

test_that(".setup_TRAILA_versions registers the 'updated' norms version", {
  expect_true("updated" %in% get_versions(TRAILA(), "norms"))
})
...
test_that(".setup_TRAILA_versions registers the 'updated_2024.06' regression version", {
  expect_true("updated_2024.06" %in% get_versions(TRAILA(), "regression"))
})

test_that(".setup_TRAILA_versions registers the 'updated_2025.06' regression version", {
  expect_true("updated_2025.06" %in% get_versions(TRAILA(), "regression"))
})

test_that(".setup_TRAILA_versions registers the 'nacc' regression version", {
  expect_true("nacc" %in% get_versions(TRAILA(), "regression"))
})
...
test_that(".setup_TRAILA_versions sets the default method to 'regression' / 'updated_2025.06'", {
  default <- get_default_method(TRAILA())

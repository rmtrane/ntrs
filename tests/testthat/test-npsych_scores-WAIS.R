...
# NOTE: .setup_WAIS_versions() does not call set_std_defaults().
# No default method test is included.

test_that("WAIS returns an object inheriting both WAIS and npsych_scores", {
  result <- WAIS()
...
test_that("WAIS sets class with WAIS first, npsych_scores second", {
  result <- WAIS()

  expect_equal(class(result), c("WAIS", "npsych_scores"))
})
...
test_that(".setup_WAIS_versions registers the 'updated_2024.06' regression version", {
  expect_true("updated_2024.06" %in% list_method_versions(WAIS(), "regression"))
})

test_that(".setup_WAIS_versions registers the 'updated_2025.06' regression version", {
  expect_true("updated_2025.06" %in% list_method_versions(WAIS(), "regression"))
})

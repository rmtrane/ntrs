...
# validate_test_scores() requires a *named* numeric vector for codes,
...
# fail until the constructor or validate_test_scores() is updated.

test_that("REYTOTAL returns an object inheriting both REYTOTAL and test_scores", {
  result <- REYTOTAL()
...
test_that("REYTOTAL sets class with REYTOTAL first, test_scores second", {
  result <- REYTOTAL()

  expect_equal(class(result), c("REYTOTAL", "test_scores"))
})
...
test_that(".setup_REYTOTAL_versions registers the expected methods", {
  methods <- get_std_methods(REYTOTAL())
...
test_that(".setup_REYTOTAL_versions registers the 'ravlt_trials' norms version", {
  expect_true("ravlt_trials" %in% get_versions(REYTOTAL(), "norms"))
})
...
test_that("'ravlt_trials' norms version data contains the expected covariate_prep_funs", {
  data <- get_version_data(REYTOTAL(), "norms", "ravlt_trials")
...
test_that(".setup_REYTOTAL_versions registers the 'updated_2024.06' regression version", {
  expect_true("updated_2024.06" %in% get_versions(REYTOTAL(), "regression"))
})

test_that(".setup_REYTOTAL_versions registers the 'updated_2025.06' regression version", {
  expect_true("updated_2025.06" %in% get_versions(REYTOTAL(), "regression"))
})

...
test_that("MOCATOTS returns an object inheriting both MOCATOTS and test_scores", {
  result <- MOCATOTS()
...
test_that("MOCATOTS sets class with MOCATOTS first, test_scores second", {
  result <- MOCATOTS()

  expect_equal(class(result), c("MOCATOTS", "test_scores"))
})
...
test_that(".setup_MOCATOTS_versions registers the expected methods", {
  methods <- get_std_methods(MOCATOTS())
...
test_that(".setup_MOCATOTS_versions registers the 'nacc' norms version", {
  expect_true("nacc" %in% get_versions(MOCATOTS(), "norms"))
})

test_that(".setup_MOCATOTS_versions registers the 'updated' norms version", {
  expect_true("updated" %in% get_versions(MOCATOTS(), "norms"))
})
...
test_that(".setup_MOCATOTS_versions registers the 'updated_2024.06' regression version", {
  expect_true("updated_2024.06" %in% get_versions(MOCATOTS(), "regression"))
})

test_that(".setup_MOCATOTS_versions registers the 'updated_2025.06' regression version", {
  expect_true("updated_2025.06" %in% get_versions(MOCATOTS(), "regression"))
})

test_that(".setup_MOCATOTS_versions registers the 'nacc' regression version", {
  expect_true("nacc" %in% get_versions(MOCATOTS(), "regression"))
})
...
test_that(".setup_MOCATOTS_versions sets the default method to 'regression' / 'updated_2025.06'", {
  default <- get_default_method(MOCATOTS())

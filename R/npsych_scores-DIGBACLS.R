...
#'
#' @keywords internal
.setup_DIGBACLS_versions <- function() {
  var_name <- NULL
  # Register norms versions for DIGBACLS
  lookup_table <- NpsychBatteryNorms::normative_summaries$nacc$DIGBACLS
  ...
  lookup_table <- NpsychBatteryNorms::normative_summaries$updated$DIGBACLS
  names(lookup_table)[which(
    names(lookup_table) %in% c("age_group", "edu_group", "educ_group")
  )] <- c("age", "educ")
  ...
  # Register regression versions for DIGBACLS
  coefs <- subset(

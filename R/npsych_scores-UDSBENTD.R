...
#'
#' @description Create a `UDSBENTD` object to hold UDSBENTD scores.
#'
#' @param scores Numeric scores.
#'
#' @returns
#' An object of class `UDSBENTD`.
#'
#' @export
UDSBENTD <- function(scores = numeric()) {
  ts <- npsych_scores(
    scores,
    label = "Benson Delay",
    range = c(0, 17),
...
#' Setup UDSBENTD method versions
#'
#' @description
...
#'
#' @keywords internal
.setup_UDSBENTD_versions <- function() {
  var_name <- NULL
  # Register norms versions for UDSBENTD
  lookup_table <- NpsychBatteryNorms::normative_summaries$nacc$UDSBENTD
  ...
  lookup_table <- NpsychBatteryNorms::normative_summaries$updated$UDSBENTD
  names(lookup_table)[which(
    names(lookup_table) %in% c("age_group", "edu_group", "educ_group")
  )] <- c("age", "educ")
  ...
  # Register regression versions for UDSBENTD
  coefs <- subset(

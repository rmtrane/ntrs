...
#'
#' @description Create a `CRAFTVRS` object to hold CRAFTVRS scores.
#'
#' @param scores Numeric scores.
#'
#' @returns
#' An object of class `CRAFTVRS`.
#'
#' @export
CRAFTVRS <- function(scores = numeric()) {
  ts <- npsych_scores(
    scores,
...
#' Setup CRAFTVRS method versions
#'
#' @description
...
#'
#' @keywords internal
.setup_CRAFTVRS_versions <- function() {
  var_name <- NULL
  # Register norms versions for CRAFTVRS
  lookup_table <- NpsychBatteryNorms::normative_summaries$nacc$CRAFTVRS
  ...
  lookup_table <- NpsychBatteryNorms::normative_summaries$updated$CRAFTVRS
  names(lookup_table)[which(
    names(lookup_table) %in% c("age_group", "edu_group", "educ_group")
  )] <- c("age", "educ")
  ...
  # Register regression versions for CRAFTVRS
  coefs <- subset(

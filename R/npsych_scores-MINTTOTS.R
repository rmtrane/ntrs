...
MINTTOTS <- new_npsych_scores(
  ...
  label = "MINT",
  domain = "Language",
  short_descriptor = "Multilingual Naming Test (MINT) - Total score",
  range = c(0, 32),
  codes = c(
    "Physical problem" = 95,
...
#'
#' @keywords internal
.setup_MINTTOTS_versions <- function() {
  var_name <- NULL
  # Register norms versions for MINTTOTS
  lookup_table <- normative_summaries$nacc$MINTTOTS
  ...
  lookup_table <- normative_summaries$updated$MINTTOTS
  ...
  # Register regression versions for MINTTOTS
  coefs <- subset(

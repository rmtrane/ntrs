#' Get `test_scores` subclasses
#'
#' @description
#' This function is a helper that lists all available subclasses, i.e. all functions created using `new_class <- function(scores) { test_scores(..., class = "new_class")}`.
#'
#' @returns
...
get_test_scores <- function() {
  ...
  from_registry <- {
    ...
    unique(unlist(lapply(registered_methods, \(m) ls(.std_versions[[m]]))))
  }
  ...
  from_s3 <- {
    ...
    setdiff(unique(classes), "test_scores")
  }

  candidates <- unique(c(from_registry, from_s3))

  Filter(.is_valid_test_scores, candidates)
}
...
#'
#' @returns
#' A logical value indicating whether `cls` is a valid subclass of "test_scores".
#'
#' @keywords internal
.is_valid_test_scores <- function(cls) {
  if (!is.character(cls)) {
    cli::cli_abort(
  ...
  inherits(obj, "test_scores") && cls %in% class(obj)
}

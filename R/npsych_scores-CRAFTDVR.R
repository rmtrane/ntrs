...
#'
#' @keywords internal
.setup_CRAFTDVR_versions <- function() {
  var_name <- NULL
  # Register norms versions for CRAFTDVR
  lookup_table <- NpsychBatteryNorms::normative_summaries$nacc$CRAFTDVR
  ...
  lookup_table <- NpsychBatteryNorms::normative_summaries$updated$CRAFTDVR
  names(lookup_table)[which(
    names(lookup_table) %in% c("age_group", "edu_group", "educ_group")
  )] <- c("age", "educ")
  ...
  # Register regression versions for CRAFTDVR
  coefs <- subset(

...
#'
#' @keywords internal
.setup_CRAFTURS_versions <- function() {
  var_name <- NULL
  # Register norms versions for CRAFTURS
  lookup_table <- NpsychBatteryNorms::normative_summaries$nacc$CRAFTURS
  ...
  lookup_table <- NpsychBatteryNorms::normative_summaries$updated$CRAFTURS
  names(lookup_table)[which(
    names(lookup_table) %in% c("age_group", "edu_group", "educ_group")
  )] <- c("age", "educ")
  ...
  # Register regression versions for CRAFTURS
  coefs <- subset(

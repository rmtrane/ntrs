...
When loading the package, messages are printed for all the defaults being set. These can be suppressed with `suppressPackageStartupMessages()`.
...
Each standardized column is an `std_npsych_scores` object carrying `@method`,
`@version`, and `@description` properties. Use `methods_from_std_data()` to
extract them conveniently:
